...
class Lrs3(tfds.core.BeamBasedBuilder):
    ...
    def _build_pcollection(self, pipeline, archive_path):
        ...
        def _read_audio(fobj):
            pydub = tfds.core.lazy_imports.pydub
            audio_segment = pydub.AudioSegment.from_file(fobj, format='mp4')
            audio_array = np.array(audio_segment.get_array_of_samples())
            audio = audio_array.astype(np.int16)  # pydub decodes 'mp4' to int16(s16le)
            return audio

        def _extract_data(inputs):
            """Extracts files from the zip archives."""
            ...
            with tf.io.gfile.GFile(archive_path, 'rb') as fobj:
                ...
                for member in z.infolist():
                    ...
                    if not video_path:
                        continue
                    video_info = member
                    yield [archive_path, video_info]

        def _process_example(inputs):
            archive_path, video_info = inputs
            with tf.io.gfile.GFile(archive_path, 'rb') as fobj:
                z = zipfile.ZipFile(fobj)

                video_path = _normpath(video_info.filename)
                ...
                audio_pcm = _read_audio(audio_fobj)
